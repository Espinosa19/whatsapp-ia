...
export default function LeadsViewer() {
 ...
 const fetchLeads = async () => {
  ...
  const setConversationMode = async (leadId, mode) => {
    try {
      await axios.put(`${API_BASE_URL}/leads/${leadId}/mode`, {
        mode // 'bot' | 'human'
      });
      // Refresca la lista de leads
      await fetchLeads();
      // Actualiza el selectedLead si es el lead actual
      if (selectedLead && selectedLead.id === leadId) {
        // Obtén el lead actualizado de la lista
        const updated = leads.find(l => l.id === leadId);
        if (updated) {
          setSelectedLead({ ...updated, conversation_mode: mode });
        } else {
          // Si no se encuentra, solo actualiza el campo conversation_mode
          setSelectedLead({ ...selectedLead, conversation_mode: mode });
        }
      }
    } catch (err) {
      console.error(err);
      alert('Error al cambiar modo de conversación');
    }
  };
const getConversationModeUI = (lead) => {
      ...
      <div className="lead-details-panel">
        {selectedLead ? (
          <>
            ...
            {/* Estado de conversación (humano/bot) */}
            <div style={{
              display: 'flex',
              alignItems: 'center',
              gap: '8px',
              ...
              <span style={{
                background: getConversationModeUI(selectedLead).bg,
                color: getConversationModeUI(selectedLead).color,
                padding: '4px 10px',
                borderRadius: '16px',
                fontWeight: 600,
                fontSize: '0.95em',
                border: `1px solid ${getConversationModeUI(selectedLead).color}`
              }}>
                {getConversationModeUI(selectedLead).label}
              </span>
            </div>
            ...
            {editingLead === selectedLead.id ? (
              ...
              <>
               ...
               <div className="details-actions">
                  ...
                  <button
                    className="btn-bot"
                    onClick={() => setConversationMode(selectedLead.id, 'bot')}
                    style={{
                      background: '#9c27b0',
                      color: 'white',
                      border: 'none',
